		...
		<div id="contgames">
			...
			<div class="games">
				...
				<?php
						...
						$querycont = mysqli_query($co, "SELECT COUNT(j.imagen) FROM juegos j, juegosfinalizados jf, usuarios u WHERE j.id=jf.id_juego && 
		    				u.id=jf.id_usuario && u.id='$_SESSION[id]'");
						$tab = mysqli_fetch_row($querycont);

						

		    			$query = mysqli_query($co, "SELECT j.imagen, j.id FROM juegos j, juegosfinalizados jf, usuarios u WHERE j.id=jf.id_juego && 
		    				u.id=jf.id_usuario && u.id='$_SESSION[id]'");
