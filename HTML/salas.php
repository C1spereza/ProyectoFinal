	...
	<div id="bigcontainer">
		<div id="controom">
			...
			<?php
				...
				$query = mysqli_query($co, "SELECT * FROM `salas`");


    			while($sala = mysqli_fetch_array($query)){


    				$querys = mysqli_query($co, "SELECT j.imagen FROM juegos j, salas s WHERE '$sala[id_juego]'=j.id");
    				$tab = mysqli_fetch_row($querys);

    				echo '<a style="text-decoration: none; color: black;" href="comentarios.php?sala='.$sala['id'].'"><div class="room">';
    				echo '<div><img class="gamer" src="data:image/jpeg;base64,'.base64_encode( $tab[0]).'"></div>';
					echo '<div class="titleroom"><h3>'.$sala["nombresala"].'</h3></div>';
					echo '</div></a>';
				}
